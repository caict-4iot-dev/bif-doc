...
| proxy      | 代理和可升级智能合约库                 | 完全支持 |
| interface  | 标准合约接口文件                       | 完全支持 |
| token      | 各类token的标准实现，如ERC721、ERC1155 | 完全支持 |
| governance | 链上治理                               | 不支持   |
| vendor     | time lock 接口                         | 完全支持 |
...
- 功能
  ...
  > **辅助功能**：
  >
  > 1. isLocked 映射：用于存储每个账户是否被锁定的状态。
  > 
  > 2. lockEnabled 变量：表示当前合约的锁定功能是否开启。
  > 
  > 3. lockedAddresses 数组：存储当前所有被锁定的账户地址。
