...
下文以部署JavaScript合约为例进行讲解，如需部署其他合约，自行替换合约即可，其余流程皆类似。

## 5.2 合约源码展示

本合约为数据存储合约，主要实现三个功能，管理员数据上报、更新；通过bid进行链上结果查询。
...
- 合约调用
  ...
  ```java
          ...
          //数据更新
  	  // String  input = "{\"method\":\"store\",\"params\":{\"id\":\"did:bid:ef8yYVtG6xqZYr7rTgsnBywTykiBg2mw\",\"data\":\"测试val2\"}}";
