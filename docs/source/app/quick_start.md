...
2.1 执行docker images查看拉取的镜像IMAGE ID
...
在镜像系统中执行上述命令后查看bif服务，如果查询不到进程，执行exit命令在宿主机再执行 **2.2 启动Quicknode服务**操作
...
​	（1）通过邮件获取**niefanjie@caict.ac.cn**；

​    （2）通过星火插件钱包申请可信凭证获取100星火令；

​    （3）星火可信企业可登录业务平台2.0申请星火令，待后续上线。
