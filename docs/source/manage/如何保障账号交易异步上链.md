...
星火链网底层区块链平台（下文简称星火链)作为“许可”公有链，拥有一部分的公有属性，使用星火令（类似于以太坊的`gas`）作为智能合约执行的消耗，同时不同账户之间可以通过交易相互转移星火令，星火链的账户模型设计了连续递增的`nonce`值的方式来防止出现重放攻击。
...
<img src="../_static/images/image-20230222105346529.png" style="zoom:80%;" />
